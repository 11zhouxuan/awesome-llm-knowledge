...
- [范式五：原生结构索引（Native Structure Indexing）](#范式五原生结构索引native-structure-indexing)
  - [视觉检索（Visual RAG）](#视觉检索visual-rag)
  - [代码索引（Code RAG）](#代码索引code-rag)
- [工业界知识库产品](#工业界知识库产品enterprise-knowledge-products)
- [结构化信息提取与全局聚合](#结构化信息提取与全局聚合structured-extraction--global-aggregation)
- [数据集与任务类型](#数据集与任务类型)
- [综述与基准](#综述与基准)
- [详细解读](#详细解读)
...
- **[Reliable Graph-RAG for Codebases: AST vs LLM](https://arxiv.org/abs/2601.08773)** (2026) — 对比三种代码 RAG 方案（向量/LLM图谱/AST图谱）。AST 图谱（DKB）在 45 题上 43 正确/0 错误，LLM 图谱 38 正确/2 错误且跳过 31% 文件。DKB 索引成本仅 2.25× 基线，LLM 图谱 19-45×。结论：代码场景 AST 确定性解析全面碾压 LLM 抽取。 [[代码]](https://github.com/Manideep-Reddy-Chinthareddy/graph-based-rag-ast-vs-llm) [[详细解读]](docs/code_rag_ast_vs_llm.md)

- **[Graphify](https://github.com/Graphify-Labs/graphify)** (Graphify Labs, 2025) — 将代码仓库转化为可查询知识图谱的产品化工具。Tree-sitter 解析 36+ 语言，零 API 调用完成代码索引；可选 LLM 语义增强层。Leiden 算法社区检测，输出交互可视化 + 报告 + JSON 图谱。集成 Claude Code, Cursor 等 20+ 平台。LOCOMO Recall@10: 0.497（vs mem0 0.048）。 [[详细解读]](docs/code_rag_ast_vs_llm.md)
...
- **[Microsoft (Fabric + Purview + GraphRAG)](https://learn.microsoft.com/en-us/fabric/)** (Microsoft) — 全家桶级智能治理体系。以 Microsoft Fabric (OneLake) 为单一数据源（Shortcuts 免搬迁），叠加 Microsoft Research 的 GraphRAG 抽取实体/关系构建全局语义网络。杀手锏是 Purview 的 DSPM for AI：多跳推理时运行时权限审计、输出自动继承敏感度标签、推理路径对审计透明。适合金融/医疗等强合规行业。 [[详细解读]](docs/microsoft_fabric_graphrag.md)

- **[Snowflake (CoWork / Cortex Agents)](https://www.snowflake.com/en/product/features/cortex/)** (Snowflake) — 主打数据重力与计算下沉，让 AI 直接进入数仓内部工作。摒弃传统 RAG 的导出-向量化流程，在数据层内运行混合搜索（向量 + 原生 SQL），支持 Deep Research 主-子代理并行工作流。强调受信任边界内计算（数据不外泄）+ 智能路由分发。适合核心业务数据托管在 Snowflake 的企业。 [[详细解读]](docs/snowflake_cortex_agents.md)

- **[Atlan (Context Graph)](https://atlan.com/)** (Atlan) — 跨云中立的上下文总线。Context Graph 只管活跃元数据（血缘、语义字典、所有权、使用规范、权限策略），不存原始数据。业内率先原生支持 MCP：Claude / OpenAI / LangChain 代理查询前先经 MCP 调取"长期记忆与治理规则"。适合数据分散在 AWS/Azure/Databricks 的多云企业。 [[详细解读]](docs/atlan_context_graph.md)

- **[Google (Vertex AI Search and Conversation)](https://cloud.google.com/enterprise-search)** (Google) — 搜索级底蕴 + 多模态融合 + 开箱即用。继承谷歌搜索算法并绑定 Gemini，只需给出 GCS 桶或内网 URL 即全托管运行，可解析文字/表格/图表。强调自动溯源（维基式角标链接直达原始 PDF 页面）与低代码。适合海量半/非结构化文档的多模态知识问答。 [[详细解读]](docs/google_vertex_ai_search.md)

---

## 结构化信息提取与全局聚合（Structured Extraction & Global Aggregation）

从文档中用 LLM 提取关键字段存入结构化数据库，使全局聚合类查询（统计/排序/筛选/TopK）可通过 SQL 精确计算，而非每次查询都去原文检索。

### 全局聚合/语料库级推理

- **[GlobalRAG](https://arxiv.org/abs/2510.26205)** (复旦, 2025) — 提出 GlobalQA 基准（13K+ QA），揭示现有 RAG 在全局聚合任务上仅达 1.51 F1。文档级检索 + 智能过滤 + 符号计算工具实现 6.63 F1（+339%）。证明纯 LLM 无法做精确统计，需要程序化工具辅助。 [[详细解读]](docs/globalrag.md)

### LLM 驱动的文档关键信息提取

- **[AgenticIE](https://arxiv.org/abs/2509.11773)** (2025) — Agent 式信息提取框架（planner-executor-corresponder 模式），从复杂监管文档中提取结构化数据。比 GPT-4o 提升 16%（Exact Match 0.396 vs 0.342）。引入高密度标注数据集（每文档 190+ 标注实体）。

- **[RASG: Retrieval Augmented Structured Generation](https://arxiv.org/abs/2405.20245)** (IEEE 2024) — 将文档信息提取重构为 LLM "工具调用"问题，涵盖关键信息提取和行项识别。LLM + RASG 匹配或超越当前多模态 SOTA。提出 GLIRM 通用行项识别指标。

- **[Source-Grounded Data Generation for Text-to-JSON](https://arxiv.org/abs/2606.20072)** (2026) — STAGE 框架：LLM 从报告自动生成 JSON schema 并填充结构化数据，支持可扩展的合成数据生成。

- **[MimirRAG](https://arxiv.org/abs/2605.25030)** (2026) — 金融数据的多 Agent RAG 框架，包含元数据提取、Agent 查询规划和混合搜索。将非结构化金融文档转化为可查询的结构化知识。

### 表单/合同/发票类提取

- **[Beyond Logprobs: Multi-Signal Confidence Engine](https://arxiv.org/abs/2606.24420)** (2026) — 发票/收据字段提取的多信号置信度引擎，解决 LLM 提取结果的可靠性评估问题。

- **[LLM-Based Examination of Securities Prospectuses](https://arxiv.org/abs/2606.27316)** (2026) — 金融招股说明书的资格条件提取管道，用于文档合规验证。

- **[Information Extraction from Electricity Invoices](https://arxiv.org/abs/2604.25927)** (2026) — 使用通用 LLM（prompt engineering）从电费发票中提取结构化数据，无需微调。
