...
- [范式一：LLM 驱动的知识提炼](#范式一llm-驱动的知识提炼llm-powered-indexing)
  ...
  - [结构化表格/数据库](#结构化表格数据库structured-extraction)
...
- **[GlobalRAG](https://arxiv.org/abs/2510.26205)** (复旦, 2025) — 提出 GlobalQA 基准（13K+ QA），揭示现有 RAG 在全局聚合任务上仅达 1.51 F1。文档级检索 + 智能过滤 + 符号计算工具实现 6.63 F1（+339%）。证明纯 LLM 无法做精确统计，需要程序化工具辅助。*（也属于范式三：智能体驱动）* [[详细解读]](docs/globalrag.md)

**LLM 驱动的文档关键信息提取：**

- **[AgenticIE](https://arxiv.org/abs/2509.11773)** (2025) — Agent 式信息提取框架（planner-executor-corresponder 模式），从复杂监管文档中提取结构化数据。比 GPT-4o 提升 16%（Exact Match 0.396 vs 0.342）。

- **[RASG: Retrieval Augmented Structured Generation](https://arxiv.org/abs/2405.20245)** (IEEE 2024) — 将文档信息提取重构为 LLM "工具调用"问题，涵盖关键信息提取和行项识别。LLM + RASG 匹配或超越当前多模态 SOTA。

- **[Source-Grounded Data Generation for Text-to-JSON](https://arxiv.org/abs/2606.20072)** (2026) — STAGE 框架：LLM 从报告自动生成 JSON schema 并填充结构化数据。

- **[MimirRAG](https://arxiv.org/abs/2605.25030)** (2026) — 金融数据的多 Agent RAG 框架，包含元数据提取、Agent 查询规划和混合搜索。
